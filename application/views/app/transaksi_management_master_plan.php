...
$arrtabledata= array(
    ...
    , array("label"=>"Kode Masterplan", "field"=> "KODE_MASTER_PLAN", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Distrik", "field"=> "NAMA_DISTRIK", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Unit", "field"=> "NAMA_UNIT", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Equipment", "field"=> "EAM_NAMA", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Judul Kegiatan", "field"=> "JUDUL_KEGIATAN", "display"=>"",  "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Status", "field"=>"STATUS_APPROVE_NAMA", "display"=>"", "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"Status", "field"=>"STATUS_APPROVE_ID", "display"=>"1", "width"=>"", "colspan"=>"", "rowspan"=>"")
    , array("label"=>"fieldid", "field"=> "PLAN_RLA_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);
...
<script type="text/javascript">
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/transaksi_management_master_plan_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/transaksi_management_master_plan_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        if(valinfostatus==1 || valinfostatus==90)
        {}
        else
        {
            if(valinfostatus == 20)
            {
                $.messager.alert('Info', "Data tidak bisa dihapus, data sudah disetujui", 'warning');
            }
            else
            {
                $.messager.alert('Info', "Data tidak bisa dihapus, data sudah dikirim untuk proses approval", 'warning');
            }
        ...
        $.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
            if (r){
                $.getJSON("json-app/plan_rla_json/delete/?reqId="+valinfoid,
                    function(data){
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/plan_rla_json/json?reqPencarian="+reqPencarian;
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/plan_rla_json/json";
