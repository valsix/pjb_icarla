...
$arrtabledata= array(
    ...
    , array("label"=>"Nama/NID", "field"=> "NAMA", "display"=>"",  "width"=>"20", "colspan"=>"", "rowspan"=>"")
    ...
    , array("label"=>"fieldid", "field"=> "CATATAN_RLA_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);
        ...
        <div class="konten-inner">
            <ul class="nav nav-pills mr-auto">
                ...
                if(!empty($reqIdRla))
                {
                    ?> 
                    <li class="nav-item " >
                        <a class="nav-link "  href="app/index/transaksi_timeline_rla?reqIdRla=<?=$reqIdRla?>&reqLihat=<?=$reqLihat?>&vstatus=<?=$vstatus?>"> &nbsp;Timelane Rla</a>
                    </li>
                    <li class="nav-item active ">
                        <a class="nav-link active " href="app/index/transaksi_catatan_rla?reqIdRla=<?=$reqIdRla?>&reqLihat=<?=$reqLihat?>&vstatus=<?=$vstatus?>"> &nbsp;Catatan/Log RLA</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link"  href="app/index/transaksi_monitoring_pengadaan?reqIdRla=<?=$reqIdRla?>&reqLihat=<?=$reqLihat?>&vstatus=<?=$vstatus?>"> &nbsp;Monitoring Pengadaan & Kontrak</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="app/index/transaksi_pengukuran?reqIdRla=<?=$reqIdRla?>&reqLihat=<?=$reqLihat?>&vstatus=<?=$vstatus?>"> &nbsp;Pengukuran</a>
                    </li>
                    <?
                    if($vstatus==20)
                    {
                        ?>
                        <li class="nav-item ">
                            <a class="nav-link " href="app/index/report_form_uji_plan_rla_dinamis?reqIdRla=<?=$reqIdRla?>&reqLihat=<?=$reqLihat?>&vstatus=<?=$vstatus?>"> &nbsp;Report</a>
                        </li>
                        <?
                    }
                    ?>
...
<script type="text/javascript">
    ...
    $("#btnEdit").on("click", function () {
        ...
        varurl= "app/index/transaksi_catatan_rla_add?reqId="+valinfoid+"&reqIdRla=<?=$reqIdRla?>";
        document.location.href = varurl;
    });

    $("#btnAdd").on("click", function () {
        btnid= $(this).attr('id');
         varurl= "app/index/transaksi_catatan_rla_add?reqId=&reqIdRla=<?=$reqIdRla?>";
        document.location.href = varurl;
    });
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/transaksi_catatan_rla_add?reqId="+valinfoid+"&reqLihat=1&reqIdRla=<?=$reqIdRla?>";
        document.location.href = varurl;
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
            if (r){
                $.getJSON("json-app/catatan_rla_json/delete/?reqId="+valinfoid+"&reqIdRla=<?=$reqIdRla?>",
                    function(data){
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/catatan_rla_json/json?reqPencarian="+reqPencarian+"&reqIdRla=<?=$reqIdRla?>";
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/catatan_rla_json/json?reqIdRla=<?=$reqIdRla?>";
