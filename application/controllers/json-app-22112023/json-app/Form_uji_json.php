...
class Form_uji_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqEquipmentId= array_filter($this->input->post("reqEquipmentId"));
	...
	function simpan_tipe()
	{
		...
		$reqId= $this->input->post("reqId");
		$reqTipePengukuranId= $this->input->post("reqTipePengukuranId");
		// print_r($reqTipePengukuranId);exit;

		$reqTipePengukuranId = explode(',', $reqTipePengukuranId);

		$reqSimpan=0;

		foreach ($reqTipePengukuranId as $key => $value) {
			$setcheck = new FormUji();
			$statement = " AND A.FORM_UJI_ID = '".$reqId."' AND A.PENGUKURAN_ID = '".$value."' ";
			$setcheck->selectByParamsCheckPengukuran(array(), -1, -1, $statement);
   			// echo $setcheck->query;exit;
			$setcheck->firstRow();
			$reqPengukuranId= $setcheck->getField("PENGUKURAN_ID");
			if(empty($reqPengukuranId))
			{
				$setpengukuran = new FormUji();
				$setpengukuran->setField("PENGUKURAN_ID", $value);
				$setpengukuran->setField("FORM_UJI_ID", $reqId);
				if($setpengukuran->insertpengukuran())
				{
					$reqSimpan= 1;
	...
	function copy()
	{
		...
		$set->setField("FORM_UJI_ID_PILIH", $reqId);
