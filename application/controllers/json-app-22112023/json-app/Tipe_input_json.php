...
class Tipe_input_json extends CI_Controller
{
	...
	function json()
	{
		...
		$sOrder = " ORDER BY A.TIPE_INPUT_ID ";
	...
	function add()
	{
		...
		$reqFunction= $this->input->post("reqFunction");
		$reqKeterangan= $this->input->post("reqKeterangan");
		$reqTipePengukuran= $this->input->post("reqTipePengukuran");
		$reqStatusTable= $this->input->post("reqStatusTable");

		$set = new TipeInput();
		$set->setField("NAMA", $reqNama);
		$set->setField("STATUS", $reqStatus);
		$set->setField("FUNCTION_INPUT", $reqFunction);
		$set->setField("KETERANGAN", $reqKeterangan);
		$set->setField("TIPE_INPUT_ID", $reqId);
		$set->setField("STATUS_TABLE", $reqStatusTable);
		...
		if($reqSimpan == 1 )
		{
			if(!empty($reqTipePengukuran))
			{
				$setinsert = new TipeInput();
				$setinsert->setField("TIPE_INPUT_ID", $reqId);
				$setinsert->deletedetail();
				foreach ($reqTipePengukuran as $key => $value) {
					
					$setinsert->setField("TIPE_PENGUKURAN_ID", $value);
					
					if($setinsert->insertdetail())
					{
						$reqSimpan= 1;
					}
