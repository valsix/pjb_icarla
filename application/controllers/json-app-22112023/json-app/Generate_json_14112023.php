...
class generate_json extends CI_Controller {
	...
	function MasterJabatan()
	{
		...
		$json = file_get_contents('https://talentman.ptpjb.com/api/daftar_jabatan_erm');
	...
	function MasterUserInternal()
	{
	    ...
	    $setInsert= new Generate();
	   	for($i=0; $i<count($jsonJadiproses); $i++){
		    ...
		    // $statement= " AND A.POSITION_ID LIKE '%".$reqPositionId."%'";
		    $statement= " AND TRIM(NID) = TRIM('".$reqNid."') AND TRIM(A.POSITION_ID) = TRIM('".$reqPositionId."')";
	...
	function work_order()
	{
		...
		$set->selectByParams(array(), -1, -1, $statement);
   		 // echo $set->query;exit;
   		$set->firstRow();
   		$reqDistrikKode= $set->getField("KODE");
		...
		if($reqEquipmentId==1)
		{
			$linkapi='http://maximo-training.ptpjb.com:9082/maxrest/oslc/os/mxwodetail?lean=1&oslc.where=spi%3Aistask%3D0%20and%20spi%3Astatus%20IN%20%5B%22APPR%22%2C%22INPROG%22%5D%20and%20spi%3Asiteid%3D%22'.$reqDistrikKode.'%22&collectioncount=1';
		}
	...
	function work_request()
	{
		...
		if($reqEquipmentId==1)
		{
			$linkapi='http://maximo-training.ptpjb.com:9082/maxrest/oslc/os/mxsr?lean=1&oslc.where=spi%3Asiteid%3D%22'.$reqDistrikKode.'%22%20and%20spi%3Astatus%20IN%20%5B%22QUEUED%22%2C%22WOCREATED%22%5D&collectioncount=1';
		}
		elseif($reqEquipmentId==2)
		{
			$linkapi='';
