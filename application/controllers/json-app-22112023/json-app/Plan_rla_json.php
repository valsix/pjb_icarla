...
class plan_rla_json extends CI_Controller
{
	...
	function add()
	{
		...
		$set->setField("STATUS_FINISH", $reqFinish);
		...
		{
			echo "xxx***Kolom Kode Master Plan tidak boleh terdapat spasi";exit;
		}
		...
		if ($reqMode == "insert")
		{
			...
			$statement=" AND A.KODE_MASTER_PLAN =  '".$reqKode."' ";
			$check = new PlanRla();
			$check->selectByParams(array(), -1, -1, $statement);
			// echo $check->query;exit;
			$check->firstRow();
			$checkKode= $check->getField("KODE_MASTER_PLAN");

			if(!empty($checkKode))
			{
				echo "xxx***Kode Master plan ".$checkKode." sudah ada";exit;	
			}
	...
	function addreport()
	{
		...
		$infolinkfile= $this->input->post("infolinkfile");
		$infolinkfilemulti= $this->input->post("infolinkfilemulti");
		// $reqLinkFile=$_FILES["reqLinkFile"];
		//print_r($reqIdDetilRla//);exit;

		// untuk validasi
		foreach ($reqNamaKolom as $key => $value) {
			$vstatustabel= $reqStatusTabel[$key];
			if($vstatustabel == "PIC")
			{
				foreach ($_FILES["reqLinkFile"]["name"][$key] as $indexfile => $vlinkfile)
				{
					$reqLinkFileType= $_FILES["reqLinkFile"]["type"][$key][$indexfile];
					if(!empty($reqLinkFileType))
					{
						$check = checkFile($reqLinkFileType,1);
						if(!$check)
						{
		...
		foreach ($reqNamaKolom as $key => $value) {
			...
			$set->setField("STATUS_TABLE", $vstatustabel);
			...
			if($vstatustabel == "PIC")
			{
				foreach ($_FILES["reqLinkFile"]["name"][$key] as $indexfile => $vlinkfile)
				{
					...
					if(!empty($reqLinkFileType))
					{
						$reqLinkFileName= $_FILES["reqLinkFile"]["name"][$key][$indexfile];
						$reqLinkFileTmp= $_FILES["reqLinkFile"]["tmp_name"][$key][$indexfile];
						...
						$infoext = pathinfo($reqLinkFileName);
						$ext= $infoext['extension'];
						$namagenerate= generateRandomString().".".$ext;

						// echo $infolinkfile[$key];exit;
						// echo $infolinkfilemulti[$key];exit;
						$lokasiBaru= $FILE_DIR.$namagenerate;
						// echo $lokasiBaru;exit;
						$prosesUpload= move_uploaded_file($reqLinkFileTmp, $lokasiBaru);
						if($prosesUpload) 
						{
							$set->setField("LINK_FILE", $lokasiBaru);
							if
							(
								empty($reqIdDetilRla[$key])
								||
								(
									empty($infolinkfile[$key]) && !empty($infolinkfilemulti[$key]) && !empty($reqIdDetilRla[$key])
								)
							)
							{
								$set->setField("LAST_CREATE_USER", $this->appusernama);
								...
								// kalau data belum ada simpan data file dengan val null
								if(empty($reqIdDetilRla[$key]) && !empty($infolinkfilemulti[$key]) && $indexfile == 0)
								{
									$set->setField("LINK_FILE", "");
									$set->insertdetil();
								}
							...
							{
								if($set->updatefiledetil())
								{
									if(file_exists($infolinkfile[$key]))
									{
										unlink($infolinkfile[$key]);
									}
									$reqSimpan= 1;
								}
							}
	...
	function hapusdata()
	{
		...
		{
			$set= new PlanRlaFormUjiDinamis();
			$set->selectseq(array(), -1,-1, " AND A.PLAN_RLA_FORM_UJI_DINAMIS_ID = ".$reqId);
			$set->firstRow();
			$seq= $set->getField("SEQ");
			$vlinkfile= $set->getField("LINK_FILE");
			...
			$set->setField("PLAN_RLA_FORM_UJI_DINAMIS_ID", $reqId);
			$set->setField("LAST_UPDATE_USER", $this->appusernama);
			$set->setField("LAST_UPDATE_DATE", 'NOW()');

			// kalau multiple maka hapus
			if($vmultipic == "multiple")
			{
				if(file_exists($vlinkfile))
				{
					unlink($vlinkfile);
				}
				$set->deletedata();
			}
			// kalau bukan maka update link file set null
			else
			{
				$set->updatelink();
			}
	...
	function delete_report()
	{
		...
		$reqPengukuranTipeInputId =  $this->input->get('reqPengukuranTipeInputId');
		...
		$set->setField("PENGUKURAN_TIPE_INPUT_ID", $reqPengukuranTipeInputId);
	...
	{	
		...
		$formuji->selectByParams($arrStatement, $rows, $offset, $statement.$statement_privacy, " ORDER BY A.ID  ASC ");
		// echo $formuji->query;exit;
		$i = 0;
		$items = array();
		while($formuji->nextRow())
		{
			...
			$row['text']			= $formuji->getField("NAMA_KELOMPOK");
			$row['FORM_UJI_ID']	= $formuji->getField("FORM_UJI_ID");
			$row['NAMA']			= "&nbsp&nbsp&nbsp&nbsp".$formuji->getField("NAMA");
			$row['NAMA_KELOMPOK']			= $formuji->getField("NAMA_KELOMPOK");
	...
	function addsummary()
	{
		...
		$reqIdRla= $this->input->post("reqIdRla");
		$reqIdRlaNameplateId= $this->input->post("reqIdRlaNameplateId");
		...
		$is_draft= $this->input->post("is_draft");
		$reqId= $this->input->post("reqId");
		$reqTestedId= $this->input->post("reqTestedId");
		$reqCoordinatorId= $this->input->post("reqCoordinatorId");
		$reqQualityId= $this->input->post("reqQualityId");
		$reqWitnessId= $this->input->post("reqWitnessId");

		$reqTestedStatus= $this->input->post("reqTestedStatus");
		$reqCoordinatorStatus= $this->input->post("reqCoordinatorStatus");
		$reqQualityStatus= $this->input->post("reqQualityStatus");
		$reqWitnessStatus= $this->input->post("reqWitnessStatus");
		...
		$set = new PlanRla();
		$set->setField("PLAN_RLA_SUMMARY_ID", $reqId);
		$set->setField("PLAN_RLA_ID", $reqIdRla[0]);
		$set->setField("V_STATUS", $is_draft);
		$set->setField("TESTED_ID", ValToNullDB($reqTestedId));
		$set->setField("COORDINATOR_ID", ValToNullDB($reqCoordinatorId));
		$set->setField("QUALITY_ID", ValToNullDB($reqQualityId));
		$set->setField("WITNESS_ID", ValToNullDB($reqWitnessId));

		$set->setField("TESTED_STATUS", ValToNullDB($reqTestedStatus));
		$set->setField("COORDINATOR_STATUS", ValToNullDB($reqCoordinatorStatus));
		$set->setField("QUALITY_STATUS", ValToNullDB($reqQualityStatus));
		$set->setField("WITNESS_STATUS", ValToNullDB($reqWitnessStatus));

		if(empty($reqId))
		{
			if($set->insertsummary())
			{
				$reqSimpan= 1;
				$reqId=$set->id;
			}
		...
		$reqSimpan=0;
		foreach ($reqNameplateId as $key => $value) {

			$set = new PlanRla();
			$set->setField("PLAN_RLA_NAMEPLATE_ID", $reqIdRlaNameplateId[$key]);
			$set->setField("PLAN_RLA_ID", $reqIdRla[$key]);
			$set->setField("NAMEPLATE_ID", ValToNullDB($value));
			$set->setField("KELOMPOK_EQUIPMENT_ID", $reqKelompokEquipmentId[$key]);
			$set->setField("PLAN_RLA_SUMMARY_ID", $reqId);
			$set->setField("SUMMARY", $reqSummary[$key]);
			$set->setField("REKOMENDASI", $reqRekomendasi[$key]);

			if(empty($reqIdRlaNameplateId[$key]))
			{
		...
		if($reqSimpan==1)
		{
			...
			{
				$infopg= $this->input->post("infopg");
				$infoketerangan= "Master Plan Summary";
				$this->load->library('libapproval');
				$vappr= new libapproval();
				$arrparam= ["ref_id"=>$reqId, "ref_tabel"=>$infopg, "infoketerangan"=>$infoketerangan];
				$vappr->approvaldata($arrparam);
