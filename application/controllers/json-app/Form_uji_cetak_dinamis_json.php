	...
	function cetak_dinamis()
	{
		...
		if(!empty($arrformuji))
		{
			foreach ($arrformuji as $key => $value) 
			{
				...
				// $statement .= " AND D.SEQ < 2";
				// $statement .= " AND D.SEQ < 5";
				...
				// $statement .= " AND A.PENGUKURAN_ID = '2'";
				...
				while($setlist->nextRow())
				{
					...
					$arrdata["JUMLAH_ROW"]=$setlist->getField("JUMLAH_ROW");
				...
				foreach ($arrisirla as $keyisi => $isiv) {
					...
					$keybarisgroup= $reqFormUjiId."-".$reqStatusTable."-".$reqMasterTabelId."-".$reqTipePengukuranId."-".$vSeq;
					$keybarisgantigroup= $reqFormUjiId."-".$reqStatusTable."-".$vSeq;
					// echo $keybarisgroup."<br/>";exit;
					...
					if($reqStatusTable == "TABLE")
					{
						...
						if(!empty($reqTabelId))
						{
							...
							$loopkolom= $arrbarisgroup[$keybarisgroup."-KOLOM"];
							// print_r($loopkolom."</br>");
							...
							for($index= 1; $index <= $maxbarisrla; $index++)
							{
								...
								foreach ($arrcheck as $vindex)
								{
									...
									// print_r($setkolom.$barisglobal."</br>");
						   ...
						   while($isimaster->nextRow())
							...
							// revisi
							if(empty($arrbarisgroup[$keybarisgantigroup."-BARIS"]) || $arrbarisgroup[$keybarisgantigroup."-BARIS"] < $barisglobal)
							{
								// echo $keybarisgroup."--".$arrbarisgroup[$keybarisgroup."-BARIS"]."--".$barisglobal."xxxxx<br>";
								$arrbarisgroup[$keybarisgantigroup."-BARIS"]= $barisglobal;
							}

							// kalau sudah tutup group maka ganti baris dengan jumlah row terbesar
							if($vseqgroupurut == 0)
							{
								$barisglobal= $arrbarisgroup[$keybarisgantigroup."-BARIS"];
							}
				...
				// echo "<br>".$barisglobal.":baris next";
				// exit;

				$barisglobal++;


				$objWorksheet->setBreak('A'. $barisglobal , PHPExcel_Worksheet::BREAK_ROW );

				$statement = " AND A.PLAN_RLA_ID = '".$reqId."' ";
				$ttd= new PlanRla();
				$ttd->selectByParamsSummary(array(), -1, -1, $statement);
				// echo  $ttd->query;exit;
				$ttd->firstRow();
				$reqTestedNama= $ttd->getField("TESTED_NAMA");
				$reqCoordinatorNama= $ttd->getField("COORDINATOR_NAMA");
				$reqQualityNama= $ttd->getField("QUALITY_NAMA");
				$reqWitnessNama= $ttd->getField("WITNESS_NAMA");
				unset($set);
				...
				$objWorksheet->setCellValue($kolomtestedawal.$barisname, $reqTestedNama);
				...
				$objWorksheet->setCellValue($kolomkordinatorawal.$barisname, $reqCoordinatorNama);
				...
				$objWorksheet->setCellValue($kolomqualityawal.$barisname, $reqQualityNama);
				...
				$objWorksheet->setCellValue($kolomwitnessawal.$barisname, $reqWitnessNama);
