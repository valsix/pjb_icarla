...
class Role_approval_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqNamaRole= $this->input->post("reqNamaRole");
		$reqDeskripsi= $this->input->post("reqDeskripsi");
		$reqStatus= $this->input->post("reqStatus");

		$set = new RoleApproval();
		$set->setField("ROLE_NAMA", $reqNamaRole);
		$set->setField("ROLE_DESK", $reqDeskripsi);
		$set->setField("ROLE_ID", $reqId);
		$set->setField("STATUS", $reqStatus);

		$reqSimpan= "";
		if ($reqMode == "insert")
		{
			$set->setField("CREATED_BY", $this->adminusernama);
			$set->setField("CREATED_AT", 'CURRENT_DATE');
			if($set->insert())
			{
				$reqSimpan= 1;
			}
		...
		{	
			$set->setField("UPDATED_BY", $this->adminusernama);
			$set->setField("UPDATED_AT", 'CURRENT_DATE');
			if($set->update())
			{
				$reqSimpan= 1;
			}
