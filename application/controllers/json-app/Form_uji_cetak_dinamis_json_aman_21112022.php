	...
	function cetak_dinamis()
	{
		...
		if(!empty($arrformuji))
		{
			...
			foreach ($arrformuji as $key => $value) 
			{
			    ...
			    $setpengukuran= new CetakFormUjiDinamis();
			    $statement = " AND A.FORM_UJI_ID = '".$reqFormUjiId."'  AND C.PLAN_RLA_ID = '".$reqId."'  AND C.KELOMPOK_EQUIPMENT_ID = '".$reqKelompokEquipmentId."'  ";

			    $setpengukuran->selectByParamsFormUjiPengukuran(array(), -1,-1,$statement);
				// echo  $setpengukuran->query;
				$tabeli=1;
				$tambahbaris=0;
				$baristext=8;
				$reqBaris=0;
			    ...
			    while($setpengukuran->nextRow())
			    {
			    	$reqTipePengukuranId= $setpengukuran->getField("PENGUKURAN_ID");
			    	
			    	$statement = " AND A.PENGUKURAN_ID = ".$reqTipePengukuranId." AND F.KELOMPOK_EQUIPMENT_ID = ".$reqKelompokEquipmentId." AND D.FORM_UJI_ID= ".$reqFormUjiId."  AND F.PLAN_RLA_ID = '".$reqId."' ";
			    	$setlist= new CetakFormUjiDinamis();
			    	$setlist->selectByParamsPengukuranTipeInput(array(), -1,-1,$statement);
					// echo  $setlist->query;
					
			    	while($setlist->nextRow())
			    	{

			    		$reqMasterTabelId= $setlist->getField("MASTER_TABEL_ID");
						...
						else if($reqStatusTable=="TEXT" && empty($reqMasterTabelId))
						{
							// print_r($reqFormUjiId);
							$kolomtextjumlah=3;
							
							$kolomtext= toAlpha($kolomtextjumlah);
							$kolomtexttitik= toAlpha($kolomtextjumlah+1);
							// print_r($kolomtext);
							$objWorksheet->setCellValue($kolomtext.$baristext, $reqValue);
							$objWorksheet->setCellValue($kolomtexttitik.$baristext, ":");




							// $statement = " AND A.PENGUKURAN_ID = ".$reqTipePengukuranId." AND F.KELOMPOK_EQUIPMENT_ID = ".$reqKelompokEquipmentId." AND D.FORM_UJI_ID= ".$reqFormUjiId."  AND F.PLAN_RLA_ID = '".$reqId."' ";
							// $settext= new CetakFormUjiDinamis();
							// $settext->selectByParamsFormUjiDetilDinamis(array(), -1,-1,$statement);
							// // echo  $setlist->query;

							// while($settext->nextRow())
							// {
							// }
							$baristext++;
