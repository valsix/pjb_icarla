...
class Import_json extends CI_Controller
{
	...
	function distrik() 
	{
		...
		$arrField= array("NAMA","KODE_DISTRIK","KODE_SITE","KODE_PERUSAHAAN");
		...
		for ($i=2; $i<=$baris; $i++){
			...
			for($row=0; $row < count($arrField); $row++){
				...
				else if($arrField[$row]=="KODE_SITE")
				{
					if (!empty($tempValue))
					{
						$statement =" AND A.KODE_SITE LIKE '%".$tempValue."%'";
						$check = new Import();
						$check->selectByParamsCheckDistrik(array(), -1, -1, $statement);
						// echo $check->query;exit;
						$check->firstRow();
						$reqDistrikId=$check->getField("DISTRIK_ID");
						$reqKodeDistrik=$check->getField("KODE_SITE");
						unset($check);

						if(empty($reqDistrikId))
						{
							$set->setField("KODE_SITE",$tempValue);
						}
					}
					else
					{
						echo "xxx***Kode Site Distrik Belum Diisi";
						exit();
					}
				}
