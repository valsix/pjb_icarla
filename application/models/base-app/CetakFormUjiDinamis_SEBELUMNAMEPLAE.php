<? 
 ...
 //    }
	...
	{
		$str = "
		SELECT A.PENGUKURAN_TIPE_INPUT_ID,D.VALUE,A.PENGUKURAN_ID,B.TABEL_TEMPLATE_ID, B.NAMA TABEL_NAMA,D.SEQ, COALESCE( NULLIF(A.STATUS_TABLE,'') , 'BINARY' ) STATUS_TABLE
		FROM form_uji_detil_dinamis A
		...
		INNER JOIN plan_rla_form_uji_dinamis F ON A.FORM_UJI_ID = F.FORM_UJI_ID  AND A.PENGUKURAN_ID = F.PENGUKURAN_ID AND D.PENGUKURAN_TIPE_INPUT_ID = F.PENGUKURAN_TIPE_INPUT_ID
		WHERE 1=1
		"; 
    ...
    function selectByParamsNameplate($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY A.NAMEPLATE_ID ASC")
	{
		$str = "
		SELECT A.*
		from NAMEPLATE A 
